...
function createApp() {
    ...
    app.get('/test-created', (req: Request, res: Response) => {
        return res.builder.as.created({ id: 42, name: 'test' }, 'Resource created').build();
    });
...
describe('BaR Express Integration', () => {
    ...
    describe('Metadata', () => {
        ...
        it('her request farklı request_id almalı', async () => {
            ...
            expect(res1.body.metadata.request_id).not.toBe(res2.body.metadata.request_id);
        });
    ...
    describe('Response Shape', () => {
        ...
        it('success flag explicit override edilebilmeli', async () => {
            const res = await request(createApp()).get('/test-explicit-success-override');
